...
static int timestamp_to_fd(struct fd_pipe *pipe, uint32_t timestamp)
{
    int fd;
    ...
    if (kgsl_pipe_safe_ioctl(pipe->dev->fd, IOCTL_KGSL_TIMESTAMP_EVENT, &event))
        return -1;
    return fd;
}

static int flush_submit_list(struct list_head *submit_list)
{
    ...
    foreach_submit_safe (submit, submit_list) {
        ...
        if (submit == last_submit(submit_list)) break;
