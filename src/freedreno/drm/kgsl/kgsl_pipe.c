...
static int wait_timestamp_safe(int fd, unsigned int context_id,
                               unsigned int timestamp, int64_t timeout_ms)
{
...
static int kgsl_pipe_get_param(struct fd_pipe *pipe, enum fd_param_id param,
                               uint64_t *value)
{
...
static int kgsl_pipe_set_param(struct fd_pipe *pipe, uint32_t param, uint64_t value)
{
...
{
    struct kgsl_pipe *kgsl_pipe = to_kgsl_pipe(pipe);
    struct kgsl_drawctxt_destroy req = { .drawctxt_id = kgsl_pipe->queue_id };
    fd_pipe_sp_ringpool_fini(pipe);
    kgsl_pipe_safe_ioctl(pipe->dev->fd, IOCTL_KGSL_DRAWCTXT_DESTROY, &req);
    free(kgsl_pipe);
}

static int kgsl_reset_status(struct fd_pipe *pipe, enum fd_reset_status *status)
{
    struct kgsl_pipe *kgsl_pipe = to_kgsl_pipe(pipe);
    ...
    if (!ret) {
        ...
        case KGSL_CTX_STAT_NO_ERROR:
            *status = FD_RESET_NO_ERROR; break;
        case KGSL_CTX_STAT_GUILTY_CONTEXT_RESET_EXT:
            *status = FD_RESET_GUILTY; break;
        case KGSL_CTX_STAT_INNOCENT_CONTEXT_RESET_EXT:
            *status = FD_RESET_INNOCENT; break;
        default:
            *status = FD_RESET_UNKNOWN; break;
        }
    }
    return ret;
}
...
struct fd_pipe *kgsl_pipe_new(struct fd_device *dev, enum fd_pipe_id id,
                              uint32_t prio)
{
    struct kgsl_pipe *kgsl_pipe = calloc(1, sizeof(*kgsl_pipe));
    if (!kgsl_pipe) {
    ...
    if (kgsl_pipe_safe_ioctl(dev->fd, IOCTL_KGSL_DRAWCTXT_CREATE, &req))
        goto fail;
