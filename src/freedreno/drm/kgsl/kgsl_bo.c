...
static uint64_t kgsl_bo_iova(struct fd_bo *bo)
{
    struct kgsl_bo *kgsl_bo = to_kgsl_bo(bo);
    return kgsl_bo->iova;
}

static void kgsl_bo_set_name(struct fd_bo *bo, const char *fmt, va_list ap)
{
    /* no-op */
}
...
void kgsl_bo_close_handle(struct fd_bo *bo)
{
    ...
    struct kgsl_gpumem_free_id req = { .id = bo->handle };
...
static void *kgsl_bo_map(struct fd_bo *bo)
{
    ...
    if (!bo->map) {
        if (kgsl_bo->bo_type == KGSL_BO_IMPORT) {
            ...
            uint64_t offset;
            if (bo->funcs->offset(bo, &offset)) return NULL;
            bo->map = os_mmap(0, bo->size, PROT_READ | PROT_WRITE, MAP_SHARED,
                              bo->dev->fd, offset);
        }
...
{
    struct kgsl_gpuobj_import_dma_buf import_dmabuf = { .fd = fd };
    ...
    if (kgsl_pipe_safe_ioctl(dev->fd, IOCTL_KGSL_GPUOBJ_IMPORT, &req)) {
        ERROR_MSG("Failed to import dma-buf (%s)", strerror(errno));
        return NULL;
    }
    struct fd_bo *bo = fd_bo_from_handle(dev, req.id, 0);
    if (bo) {
        struct kgsl_bo *kgsl_bo = to_kgsl_bo(bo);
...
static int dma_heap_alloc(uint64_t size)
{
    ...
    if (dma_heap < 0) {
        ...
        int ret = kgsl_pipe_safe_ioctl(ion_heap, _IOWR('I', 0, struct ion_allocation_data), &alloc_data);
        close(ion_heap);
        if (ret) return -1;
        ...
        int ret = kgsl_pipe_safe_ioctl(dma_heap, DMA_HEAP_IOCTL_ALLOC, &alloc_data);
        close(dma_heap);
        if (ret) return -1;
        return alloc_data.fd;
    }
...
static struct fd_bo *kgsl_bo_new_dmabuf(struct fd_device *dev, uint32_t size)
{
    ...
    struct fd_bo *bo = kgsl_bo_from_dmabuf(dev, fd);
    close(fd);
    return bo;
}
...
struct fd_bo *kgsl_bo_new(struct fd_device *dev, uint32_t size, uint32_t flags)
{
    ...
    struct kgsl_gpumem_alloc_id req = {
        .size = size,
        .flags = (flags & FD_BO_GPUREADONLY) ? KGSL_MEMFLAGS_GPUREADONLY : 0,
    };
    if (kgsl_pipe_safe_ioctl(dev->fd, IOCTL_KGSL_GPUMEM_ALLOC_ID, &req)) {
        ERROR_MSG("GPUMEM_ALLOC_ID failed (%s)", strerror(errno));
        return NULL;
    }
    struct fd_bo *bo = kgsl_bo_from_handle(dev, size, req.id);
    if (bo) {
        struct kgsl_bo *kgsl_bo = to_kgsl_bo(bo);
